...
export const load: PageServerLoad = async ({ params, fetch }) => {
  ...
  let character = await toJson<ICharacter>(fetch(`/api/characters/${id}?expand=${expand}`));
  if (!character) character = await toJson<ICharacter>(fetch(`/api/custom/${id}?expand=${expand}`));
